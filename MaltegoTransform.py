...
class MaltegoTransform(object):
	...
	def parseArguments(self,argv):
        ...
        # Michael Henriksen: Bug fix: The simple splitting fails
        # if the tweet content contains a hashtag. Only split on
        # pound symbols that are not preceded by a backslash...
				# vars = argv[2].split('#');
				vars = re.compile('(?<!\\\)#').split(argv[2])
